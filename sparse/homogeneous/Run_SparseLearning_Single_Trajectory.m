...
%[result{k}.eLS, result{k}.eSLS, result{k}.eLASSO] = errornorms_phis(sysInfo,idenLS,idenSLS,idenLASSO,rho_emp,rspan);
...
% 
% for k=1:num_trials
%      errLASSO(k) =result{k}.eLASSO(:,1);
%      errSLS(k) =result{k}.eSLS(:,1);
%      errLS(k) =result{k}.eLS(:,1);
% 
% end
% 
% pLasso1 = length(find(errLASSO<1e-4))./num_trials;
% pLasso2 = length(find(errLASSO<0.01))./num_trials;
% 
% 
% pLS1 = length(find(errLS<1e-4))./num_trials;
% pLS2 = length(find(errLS<0.01))./num_trials;
% 
% 
% pSLS1 = length(find(errSLS<1e-4))./num_trials;
% pSLS2 = length(find(errSLS<0.01))./num_trials;
% 
% 
% filename = strcat(sysInfo.name, 'M',num2str(obsInfo.M),'L',num2str(length(obsInfo.time_vec)));
% save(filename,'pLasso1','pLasso2','pLS1','pLS2','pSLS1','pSLS2','errLASSO','errLS','errSLS');

% % choose the estimators with best performance
% 
% newintial = zeros(sysInfo.d*sysInfo.N, 10);
% for l=1:1
%     newintial(:,l)=sysInfo.mu0();
% end
% 
% if eLS(1)==min([eLS(1),eSLS(1),eLASSO(1)])
%     flag=1;
%     LS_traj = construct_and_compute_traj(sysInfo,idenLS,obsInfo,solverInfo, xpath_train(:,1,:));
%     LS_traj_test = construct_and_compute_traj(sysInfo,idenLS,obsInfo,solverInfo, newintial);
% end
% 
% if eSLS(1)==min([eLS(1),eSLS(1),eLASSO(1)])
%     flag=2;
%     SLS_traj = construct_and_compute_traj(sysInfo,idenSLS,obsInfo,solverInfo, xpath_train(:,1,:));
%      SLS_traj_test = construct_and_compute_traj(sysInfo,idenSLS,obsInfo,solverInfo, newintial);
% end
% 
% if eLASSO(1)==min([eLS(1),eSLS(1),eLASSO(1)])
%     flag=3;
%     LASSO_traj = construct_and_compute_traj(sysInfo,idenLASSO,obsInfo,solverInfo, xpath_train(:,1,:));
%     LASSO_traj_test = construct_and_compute_traj(sysInfo,idenLASSO,obsInfo,solverInfo, newintial);
% end
% 
%     
...
% 
% newintial = zeros(sysInfo.d*sysInfo.N, 1);
% for l=1:1
%     newintial(:,l)=sysInfo.mu0();
% end
% 
...
%     
%     case 1
%     visualize_trajs_1D(sysInfo,idenLS,obsInfo,solverInfo,xpath_train);
%     case 2
%     visualize_trajs_1D(sysInfo,idenSLS,obsInfo,solverInfo,xpath_train);
%     case 3
%     visualize_trajs_1D(sysInfo,idenLASSO,obsInfo,solverInfo,xpath_train);
% end
% 
%     
% elseif sysInfo.d ==2

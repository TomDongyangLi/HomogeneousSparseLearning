...
% modified from Ming Zhong's code
% last update by Sui Tang  Aug-2019
function visualize_trajs_2D(sysInfo,Iden,obsInfo,solverInfo,xpath_train)
...
data = xpath_train;
IC=data(:,1,1);
result               = construct_and_compute_traj(sysInfo,Iden,obsInfo,solverInfo, IC);
...
%% downsampling the trajectories
%coarse_time_vec    = insert_time_vec(coarse_time_vec, obsInfo.T_L);
% for ind = 1 : length(chosen_dynamics)
%   trajsnoise{ind}       = deval(chosen_dynamics{ind}, fine_time_vec);
%   dyn_markers{ind} = deval(chosen_dynamics{ind}, coarse_time_vec);
% end
    ...
    for k = 1 : sysInfo.type
        ...
        for agent_ind = 1 : N_k
            ...
            if line_style_ind < 3
               ...
               % c1_at_t_2                = X_cds2{1}(agent, :);

                c2_at_t_1                = X_cds1{2}(agent, :);
                %c2_at_t_2                = X_cds2{2}(agent, :);

            elseif line_style_ind == 3
                c1_at_t_1                = [X_cds1{1}(agent, :), NaN * ones(1, plot_info.c_len)];
                %c1_at_t_2                = [X_cds2{1}(agent, :), NaN * ones(1, plot_info.c_len)];

                c2_at_t_1               = [X_cds1{2}(agent, :), NaN * ones(1, plot_info.c_len)];
                %c2_at_t_2               = [X_cds2{2}(agent, :), NaN * ones(1, plot_info.c_len)];

            end
            
            
            
          %p_handle0                 = plot([c1_at_t_1, NaN],[c2_at_t_1, NaN],'k+');
%          p_handle                  =  plot([c1_at_t_2, NaN],[c2_at_t_2, NaN],'b-');
%           
...
%       c1_conf = [c1_at_t_2, c1_at_t_2(end:-1:1)] ;
%       c2_conf = [c2_at_t_2+10*C agents_traj2(:,end:-1:1)-10*C(:,end:-1:1)];
%       p = fill(time_vec_conf,yconf,'red');
%       for q=1:length(p)
%       p(q).FaceColor = [1 0.8 0.8];
%       p(q).EdgeColor = 'none';
%       p(q).FaceAlpha = 0.5;
%       end

...
function visualize_trajs_1D(sysInfo,Iden,obsInfo,solverInfo, xpath_train)
...
%C = result.covmatrix; % Nd x time_steps x M
...
%       time_vec_conf = [time_vec, time_vec(end:-1:1)] ;
%       yconf = [agents_traj2+2*sqrt(C) agents_traj2(:,end:-1:1)-2*sqrt(C(:,end:-1:1))];

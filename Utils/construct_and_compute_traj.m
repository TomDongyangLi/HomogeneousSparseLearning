...
for m = 1 : size(ICs, 2)
 ...
 sol_true = ode15s(myODE,solverInfo.time_span,ICs(:,m),solverInfo.option); % solu from adaptive solver
 traj_true(:,:,m)  =deval(sol_true,test_time_vec) ;% Nd x steps
 
 sol_hat = ode15s(myODE1,solverInfo.time_span,ICs(:,m),solverInfo.option); % solu from adaptive solver
